...
Bitmap::Bitmap(string name)
{
  ...
  if (inFile) {
    ...
    /**************************************************************************************************************************************
      Its possible to use a buffer with inFile.read(tempFile, fileSize), but tempFile must be a char*, and resulting data has some 
      extraneous manipulations and memory allocations. It is also possible for data to be altered in subtle ways due to the buffer storage. 
      get() is reliable and consistent in that it get exactly one character, including whitespace and zeros, and can be immediately byte-
      shifted for direct storage into the header structure.
    ...
    ***************************************************************************************************************************************/
    //read char, bitshift, and assign. Unless noted, all variables are unsigned ints
    //bmp ---------------------------------------------------------------------------------------------------------------------------------
    bmpHeader.fileSignature[0] = inFile.get(); //char 
    bmpHeader.fileSignature[1] = inFile.get(); //char
    bmpHeader.bmpFileSize      = inFile.get() + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    bmpHeader.unused_1         = inFile.get() + (inFile.get() << BIT_SHIFT_8); //unsigned short
    bmpHeader.unused_2         = inFile.get() + (inFile.get() << BIT_SHIFT_8); //unsigned short
    bmpHeader.pixelOffset      = inFile.get() + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    
    //dibHeader --------------------------------------------------------------------------------------------------------------------------------   
    dibHeader.numBytesDIBHeader    = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.widthBitmapInPixels  = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.heightBitmapInPixels = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);     
    dibHeader.numColorPlanes       = (inFile.get()) + (inFile.get() << BIT_SHIFT_8); //unsigned short
    dibHeader.numBitsPerPixel      = (inFile.get()) + (inFile.get() << BIT_SHIFT_8); //unsigned short
    dibHeader.typeCompression      = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.sizeRawDataInPixelArray = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24); 
    dibHeader.horizPhysicalResolution = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.vertPhysicalResolution  = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.numColorsInPalette  = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.importantColors     = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.redChannelBitMask   = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.greenChannelBitMask = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.blueChannelBitMask  = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.alphaChannelBitMask = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.typeOfColorSpace    = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    for(int i = 0; i < 9; i++)
      dibHeader.colorSpaceEndPoints[i] =(inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.redGamma   = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.greenGamma = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
    dibHeader.blueGamma  = (inFile.get()) + (inFile.get() << BIT_SHIFT_8) + (inFile.get() << BIT_SHIFT_16) + (inFile.get()<< BIT_SHIFT_24);
...
void Bitmap::insertRow (int rowToInsert)
{
	...
	cout << dec;
	if (DEBUG_BMP) {cout << "The current size of the vector grid is " << grid.size() << endl; } 
	
	grid.insert(grid.begin() + (rowToInsert * totalColCount), totalColCount, color.pixelColor);
	totalRowCount++;
		
	if (DEBUG_BMP) {cout << "After insertion, the new size of the vector is " << grid.size() << endl;}
...
void Bitmap::deleteRow(int rowToDelete)
{
  ...
  if (DEBUG_BMP) {cout << "After deletion, the new size of the vector is " << grid.size() << endl;}
